...
parser.add_argument('-sigma'   , type=float, default=1           , help='Sigma to call run_nm. DEFAULT: 1')
parser.add_argument('-scale'   , type=float, default=1           , help='Scale factor to call run_nm. DEFAULT: 1')
...
if __name__ == "__main__":
  ...
  if not no_nm:
    run_one_nm(True, sigma, scale_factor, mdpdir, "nm_output")

  save_spectrum(molecule_path1, molecule_name1, linear1, log1, 
                molecule_path2, molecule_name2, linear2, log2, 
                start, stop, step_size, gamma, outdir, outname, 
                header, generate_csv, generate_png, generate_pdf, 
                generate_svg)
